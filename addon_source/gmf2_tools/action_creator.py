...
# Change this later
def convert_channel_index(chan_index, up_axis):
...
class GA2ActionCreator(Operator):
    ...
    def create_action(self, context, end_frame, anim_name, anim_objects):
        ...
        for anim_obj in anim_objects:
            anim_obj_datas.append(AnimObjInfo(anim_obj.obj))
            ...
            if a_obj_data is not None:
                if a_obj_data.has_anim_data:
                    if a_obj_data.has_pos:
                        GA2ActionCreator.insert_channel(self, context, a_obj_data.pos_x_block, 'POS_X', bone)
                        GA2ActionCreator.insert_channel(self, context, a_obj_data.pos_y_block, 'POS_Y', bone)
                        GA2ActionCreator.insert_channel(self, context, a_obj_data.pos_z_block, 'POS_Z', bone)

                    """if a_obj_data.has_rot:
                        GA2ActionCreator.insert_channel(self, context, a_obj_data.rot_x_block, 'ROT_X', bone)
                        GA2ActionCreator.insert_channel(self, context, a_obj_data.rot_y_block, 'ROT_Y', bone)
                        GA2ActionCreator.insert_channel(self, context, a_obj_data.rot_z_block, 'ROT_Z', bone)"""
    ...
    def insert_channel(self, context, blockData, channelName, bone):
        ...
        if channel_index <= 3:  # Position
            converted_chan_idx = convert_channel_index(channel_index, self.up_axis)
            ...
            for kf in anim_curve.keyframe_points:
                ...
                try:
                    GA2ActionCreator.insert_keyframe(self, context, bone, blockData.v_divisor, blockData.data_pairs[kf_index], channel_index)
                except IndexError:
                    print(f"Error when inserting keyframes on bone {bone.name}! Index {kf_index} was too big for our data pairs array. The length of the array is {len(blockData.data_pairs)}, and the contents are: {blockData.data_pairs}")
                    print(f"The number of keyframe points we were trying to overwrite is: {len(anim_curve.keyframe_points)}")
                    return

                kf_index += 1
        """elif channel_index <= 6:  # Rotation
            converted_chan_idx = convert_channel_index(channel_index, self.up_axis)

            if anim_curve is None:
                bone.rotation_euler = (0.0, 0.0, 0.0)
                bone.keyframe_insert(data_path="rotation", frame=1, index=converted_chan_idx)

                for curve in anim_action.fcurves:
                    if curve.data_path == f'pose.bones["{bone.name}"].rotation':
                        pass"""
    ...
    def insert_empty_keyframes(self, context, bone, kf_count, channel_index):
        ...
        converted_chan_index = convert_channel_index(channel_index, self.up_axis)
    ...
    def insert_keyframe(self, context, bone, v_div, posData, channel_index):
        if channel_index <= 3:
            ...
            """if v_div == 5:
                bone.location[converted_chan_index] = (posData / pow(2, v_div)) * 0.01
            else:
